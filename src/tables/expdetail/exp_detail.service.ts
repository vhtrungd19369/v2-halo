...
export class ExportDetailService {
    constructor(@InjectModel('Expdetail') private readonly expDetailModel:Model<ExpDetailInterface & Document>) {}
        ...
        async findOne(id: string): Promise<ExpDetailInterface> {
            return await this.expDetailModel.findOne({_id: id});
        } 

        async create(createExpDetailDto: CreateExpDetailDto): Promise<ExpDetailInterface> {
        ...
        async delete(id: string): Promise<ExpDetailInterface> {
            return await this.expDetailModel.findByIdAndDelete(id);
        }

        async update(id: string, expDetailInterface: ExpDetailInterface): Promise<ExpDetailInterface> {
            return await this.expDetailModel.findByIdAndUpdate(id, expDetailInterface, {new: true});
        }
